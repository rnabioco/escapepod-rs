...
/// End-to-end parity of the two `demux detect --method cnn` code paths: the CPU
/// path runs `AdapterCnn` (tract-onnx) per read; `--gpu` runs `GpuCnn` batched.
/// This gates the CLI's GPU path against its CPU path directly (skips without a
/// GPU or the gitignored ONNX/weights blobs — i.e. runs on GPU-node test runs,
/// not GitHub Actions).
#[test]
fn gpu_cnn_matches_tract() {
    let (weights, onnx) = (weights_path(), onnx_path());
    if !weights.exists() || !onnx.exists() {
        eprintln!("[gpu_cnn] skipping — missing {weights:?} / {onnx:?}");
        return;
    }
    let Some(gpu) = try_gpu(&weights) else { return };
    ...
    let cpu_out: Vec<usize> = reads
        .iter()
        .map(|s| tract.detect_adapter_end(s).expect("tract"))
        .collect();
    ...
    for (i, (&g, &c)) in gpu_out.iter().zip(cpu_out.iter()).enumerate() {
        if g == c {
            ...
            panic!("read {i}: gpu={g} tract={c} differ by > {ds} samples");
        }
    }
    assert!(
        exact >= reads.len() - 3,
        "too many non-exact GPU-vs-tract matches: {exact}/{}",
        reads.len()
    );
}
