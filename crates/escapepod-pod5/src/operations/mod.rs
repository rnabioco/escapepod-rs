//! High-level operations on POD5 files.
//!
//! This module provides functions for common POD5 operations like
//! filtering reads by ID, repacking files, and parsing CSV mappings for subsetting.

mod annotate;
pub(crate) mod csv_utils;
mod filter;
mod repack;
mod split;
mod subset;

pub use annotate::{
    AnnotateOptions, AnnotateResult, CollectionResult, ColumnStat, ColumnValues, ColumnWrite,
    ColumnsResult, DesignOptions, DesignResult, SidecarColumn, read_annotation, read_columns,
    read_design, read_score, read_sidecar_for, remove_annotation, remove_design, write_annotation,
    write_collection_columns, write_columns, write_design,
};
pub use filter::{
    FilterCriteria, FilterOptions, FilterResult, SubsetOutcome, filter_files,
    filter_files_with_criteria, read_ids_from_file, subset_file, subset_files,
};
pub use repack::{RepackOptions, RepackResult, repack_files};
pub use split::parse_barcode_mapping;
pub use subset::parse_csv_mapping;
