...
//! [`phase`] to start/rotate a phase; [`report`] closes whatever phase is
//! still open and, when the caller's `--profile` flag is set, prints a
//! breakdown to stderr — otherwise the timer is a cheap no-op beyond the
//! `Instant::now()` calls.
//!
