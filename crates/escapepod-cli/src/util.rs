...
pub fn collect_pod5_inputs(inputs: &[PathBuf]) -> anyhow::Result<Vec<PathBuf>> {
    ...
    // Dedupe, preserving the order the caller gave. Inputs are allowed to
    // overlap (a directory plus a file inside it, or the same path twice), and
    // processing a read twice is a silent duplication rather than an error
    // anyone would notice downstream.
    //
    // Deliberately NOT sorted: `merge`, `repack` and `index` already take this
    // path, and argument order is the order their output is written in. Sorting
    // would quietly reorder existing callers' output to fix a problem they do
    // not have.
