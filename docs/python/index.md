...
| [Sidecar annotations](reading.md#sidecar-annotations) | `Reader.annotation()` / `annotation_names()` / `design()` / `score()` / `score_names()` — demux barcodes, experimental designs, and numeric scores from the `.p5s` sidecar |
