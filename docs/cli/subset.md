...
![escpod subset](../images/subset.gif)

## Usage

```bash
escpod subset --csv <FILE> -o <OUTPUT_DIR> [OPTIONS] <INPUT>
```
...
```bash
escpod subset experiment.pod5 --csv mapping.csv -o demultiplexed/
```
...
```bash
escpod subset pooled.pod5 --csv sample_mapping.csv -o samples/
```
