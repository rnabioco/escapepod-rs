...
use escapepod::operations::{filter_files, parse_barcode_mapping, FilterOptions};
...
pub fn run(args: DemuxArgs) -> anyhow::Result<()> {
    match args.command {
        ...
        DemuxCommand::Split(split_args) => run_split(split_args),
    }
}
...
/// Run the split subcommand - split reads into separate POD5 files by barcode.
fn run_split(args: SplitArgs) -> anyhow::Result<()> {
    ...
    let mut barcode_groups: HashMap<String, HashSet<Uuid>> = HashMap::new();
    ...
    let mut barcode_stats: Vec<(String, u64, u64)> = Vec::new(); // (barcode, read_count, file_size)
    ...
    for barcode in &barcodes {
        ...
        barcode_stats.push((barcode.clone(), result.matched_reads, file_size));
    ...
    for (barcode, read_count, file_size) in &barcode_stats {
    ...
    let total_reads: u64 = barcode_stats.iter().map(|(_, count, _)| count).sum();
    let total_size: u64 = barcode_stats.iter().map(|(_, _, size)| size).sum();
