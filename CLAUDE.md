...
- **escapepod-align**: all-against-all read-to-reference alignment for small panels — the engine behind `escpod align` (#395). A leaf crate on purpose: **std only, no noodles, no git dependency**, so unlike `escapepod-demux` it stays `cargo publish`-able; all BAM/FASTA/FASTQ I/O is the CLI's, as with `escapepod-classify`. Written clean-room from the issue, with gpu-tRNA-mapper as the *functional* model only (every read against every reference, ties explicit, tracebacks for winners only) — none of its source was read, and its semi-global definition is deliberately not reproduced. Layers: `alphabet` (A/C/G/T, U folded to T, case ignored; **every other letter is a wildcard that scores as a match**), `Scoring`/`Mode` (a gap of length k is `gap_open + (k-1)·gap_extend` — parasail's convention with signs, so bwa's `-O1 -E1` is `-2,-1` here; `Local` = Smith–Waterman, `SemiGlobal` = overlap, leading/trailing gaps of *either* sequence free = parasail `sg`), `scalar` (the Gotoh oracle, score-only and with traceback), `simd` (the kernels), `sam` (CIGAR + `MD`/`NM`), and `Aligner` (panel + dispatch + tie rules). **Integer scoring throughout, so every path is tested against the scalar oracle by equality — no tolerance anywhere** — and the oracle against parasail (`tests/parasail_golden.rs`: scores *and* coordinates, which only works because the tie-breaks were measured and matched: end cell = smallest reference end then smallest read end, traceback prefers diagonal > insertion > deletion, extend > open, local stops at the first zero; CIGARs are checked by re-scoring, since optimal paths are not unique).

  **Two kernel families, both inter-sequence.** The *score* kernels (`simd::{avx2,avx512}::score_group`) put one **reference** per i16 lane (16 / 32), references sorted longest-first into lane groups with a per-group profile `s(read code, ref[j])`; `j` outer, read `i` inner, so a column's five profile vectors stay in L1, and a per-column lane mask keeps padding out of the best. **i16 is exact, not approximate**: every real cell value is bounded by `min(n, L)·max(|match|,|mismatch|)` plus one gap, `simd::fits_i16` checks that per read (and falls back to scalar), and `i16::MIN` is a −∞ that saturation keeps −∞. The *traceback* kernels (`trace_pairs`) put one **(read, reference) pair** per lane, so the unique winners of a whole chunk of reads and the members of one read's tie set fill lanes alike (`Aligner::map_reads`; the CLI feeds it 64 reads at a time); they write the scalar path's traceback byte per lane, track each lane's end cell in the scalar scan order, and hand every lane to the *same* `scalar::walk`, so their output is the oracle's by construction. That second family is not a nicety: on a real sample ~5% of reads are adapter-only and tie with **all 164** references of the adapter-flanked sacCer3 panel, and tracing ties then unique winners one reference at a time was 65%, then 19%, of total CPU — the score kernel is now ~77%. Dispatch is `escapepod_signal::lstm`'s pattern: runtime `is_x86_feature_detected!` (never a baseline bump), kernels `#[inline(never)]`, `ESCAPEPOD_ALIGN_BACKEND=scalar|avx2|avx512` caps it, and the equality tests (`avx2_matches_scalar_random`, `avx512_matches_scalar_random`, `simd_matches_scalar_on_fixture_reads`, `traceback_kernels_match_scalar`) sweep `Backend::available()`, not the dispatch's pick — run them on `rna` (Cascade Lake) or the AVX-512 kernels never execute.

  **The CUDA score kernel (`gpu` feature, `cuda/`, #401).** The only optional dependency (cudarc, `dynamic-loading`, the workspace pin); the CLI's atomic `gpu` forwards to it. Mirrors `escapepod_signal::dtw::cuda`: kernel source as a string, NVRTC at `GpuScorer::new`, which checks `is_culib_present` for the driver *and* NVRTC first so a missing library is an error rather than cudarc's panic (release aborts on panic). Layout: **one thread per (read, reference) pair**; a block is one group of references (lanes = references rounded to a warp, ≤ 256, shrunk until the panel fits 48 KB of shared memory), the grid is `(persistent blocks, groups)`, blocks pull reads from an atomic counter in longest-first order; the **panel lives in shared memory at 4 bits a base** (A/C/G/T one bit, anything else 0xF, so "masks intersect" *is* the wildcard rule), transposed so lane `t`'s word `w` is `sp[w * lanes + t]`; each thread walks its own reference to its own length (no column mask). The read is walked in **16-row stripes held in registers**, the scalar recurrence verbatim in `int` with the same `NEG`; only the stripe's bottom row (`H`, `F` as two packed `i16`) leaves registers, into a global scratch interleaved by thread (`scratch[col * total_threads + thread]`, coalesced) — 0.5 B of traffic per cell. `__launch_bounds__(256, 4)` (64 registers; the JIT's own 88 gave three blocks per SM, 6.4 s against 5.9 s on M1; 56 and 40 spill and lose). The result is an `i16` `[queries × references]` `ScoreMatrix`, a plain type compiled in every build. **Caps:** a read over `MAX_READ_LEN` = 4096 nt or outside `fits_i16` comes back unscored (the CPU scores it) — a *scheduling* bound, not a memory one: one block owns a read serially, and a 400 kb read on the GPU (tried) took the M1 kernel time from 6 s to 17.5 s; a reference over `MAX_REF_LEN` = 3072 (one warp's panel in 48 KB) refuses the scorer. Pinned by **equality** against `scalar::score` (`tests/gpu_parity.rs`: the SIMD tests' random generator in both modes and all three schemes, every read length 1–40 around the stripe, a 300-reference panel that splits into groups, the fixture reads against both fixture panels — 108,624 pairs), skipping with a message where there is no device. `examples/gpu_score_probe.rs` times `score_batch` alone on real reads (`samtools view | cut -f10`): 2.4e11 cells/s on an A30, ~20 integer ops per cell against Ampere's 64 INT32 lanes/SM/clk, i.e. ALU-bound; the 164-reference panel leaves 28 of 192 lanes idle. Not done: mixing FP32 warps (exact on integers < 2^24) with INT32 ones to use both pipes.

  **`MD`/`NM` follow `samtools calmd`, not the scoring** (`sam.rs`, pinned by `tests/sam_fields.rs` against a golden measured on samtools 1.23.1 by `tests/fixtures/gen_calmd_golden.py`): a reference `N` (or any IUPAC code) is an `MD` mismatch against every read base, read `N` included, and counts toward `NM`; letters are upper-cased (`n`→`N`) but otherwise verbatim (a reference `U` stays `U`, and `U` equals `T` for matching). This matters because `escpod classify`'s windowed bundle rebuilds the read's reference from `MD` — the #306 reference-`N` trap from the other side. The scorer still calls that `N` a match: `AS` and `MD` disagree about it on purpose.
- **escapepod-cli**: the `escpod` CLI binary (built by the default `cli` feature) plus an optional umbrella library (imported as `escapepod_cli`) that re-exports `pod5`/`signal`/`demux` behind feature flags for `default-features = false` consumers. Demux is part of the default `cli` feature (it adds no third-party crates — ndarray/serde_json/rayon/uuid are already in the graph), as is `cnn-detect` (it does add tract-onnx, ~+21 MB stripped, but only to the binary — library consumers must already use `default-features = false` to avoid clap/noodles, so they never pay for it; `--method cnn` is what the published barcode models were trained against). `default-features = false` consumers can still select either explicitly. Forward features `train` and `gpu` propagate to the demux crate (and `gpu` to `escapepod-align/gpu` and `escapepod-classify/cuda`) and remain opt-in because they need extra toolchains or a CUDA runtime. `gpu` is the only GPU feature here — there is no way to build half a GPU binary. (`escapepod-signal/gpu` stays narrower on purpose: cudarc DTW only, no onnxruntime.)
...
- `crf/encoder.rs` (feature: `crf-decode`): the ONNX encoder, dispatched to a native kernel when the export matches and to tract otherwise. Contract is `[batch,1,chunk] -> [chunk/stride, batch, n_score]`, **time-major** — the opposite of `adapter_cnn.rs`'s batch-major `[B,2,L]`, so it needs its own load-time shape probe rather than reusing that one. Standardisation constants come from the export's `metadata.json` sidecar and are **not** in `config.toml` — that file carries SeqTagger's unrelated 80.876/17.270, which silently degrades the decode.

  **The native path (`crf/encoder_native.rs`, #331 part 2).** `Recognized::from_proto` recognises exactly one exported shape — 3 `Conv`+SiLU stages, 5 stacked unidirectional LSTM layers (alternating direction via bonito's reverse trick: an ONNX-forward `LSTM` wrapped in `Slice(steps=[-1])` before and after, not a `direction="reverse"` attribute), a linear head with `tanh`, and the blank edge inserted via a `Pad` — and refuses everything else, falling back to tract with the reason logged at debug level (`ESCAPEPOD_CRF_DEBUG_RECOGNIZER=1` also prints it). The recognizer had never actually matched the shipped bundle before four real bugs were found and fixed, each confirmed against the real export's ONNX graph: the `Conv` output's SiLU (`x * sigmoid(x)`) has *two* consumers (`Sigmoid` and the `Mul`) where the matcher assumed one; a layer's reverse-input detection couldn't tell "this layer reverses" from "the previous layer's un-reverse `Slice` happens to feed me" by producer type alone (needs direct name equality checked first); the output-side mirror of that same ambiguity; and `trace_through_reshapes` took the sole `Reshape` consumer of a node that in the real export also feeds a `Shape` node (the dynamic-shape subgraph), so it needs to filter by op type. A numeric self-check at load time (`self_check`, tolerance 0.05) runs the lifted native weights against tract on random input and refuses to load the native path if they disagree — the guard against a recognizer that matches a shape without matching its semantics. The recurrence itself runs through `escapepod_signal::lstm` (shared with `escapepod-classify`'s charging net); this crate supplies the conv stages (a polyphase decomposition of the strided conv — splitting each input channel into `stride` phase sub-arrays turns the strided gather into a contiguous read, and was the fix that got native below tract at all) and the linear head (vectorised `tanh` via `lstm::tanh_slice`).

  Performance is real but modest, well short of the ~3–6× the issue estimated: `encode/native/1` (one read) is **~1.07× tract** (17.12 ms vs 18.39 ms, ldx32 bundle, T=300, one core, rna) and `encode/native/8` (`CrfEncoder::encode_group`, `preferred_batch()` reads sharing weight-row loads the way the charging kernel's `logits_batch` does) is **~1.27×** (14.5 ms/read). The estimate assumed the CRF stack pays the same L2-bandwidth-bound cost per timestep as the charging BiLSTM; it doesn't — `input_contribution`'s cost scales with `n_in`, and later layers here take `n_in = 96` (an LSTM layer's own hidden width) against the charging net's small fixed per-base feature width, so a much larger share of the CRF stack's cost is inherent to the shape rather than tract overhead. `CrfEncoder::encode_group`/`preferred_batch` feed `escpod demux`'s fused CPU pipeline (`produce_cpu_crf`/`produce_cpu_crf_multi`, groups of `CRF_ENCODE_GROUP = 8`); `encoder_backend()` reports which path loaded (`"native (scalar|avx2|avx512)"` or `"tract"`) and `ESCAPEPOD_CRF_TRACT=1` forces tract regardless. Validated end to end on the 20k dual-axis set (`data/bench/dual_axis_20k/`): 0 of 20,000 calls differ between the native and tract paths, 96.9% accuracy against ldx truth on both.
...
- `align`: all-against-all alignment of reads to a small reference panel (tRNA) — `escpod align reads.ubam -r ref.fa -o out.bam`. Takes dorado's uBAM (or forward-strand aligned BAM, or FASTA/FASTQ, plain or gzip — sniffed from content), writes records in **input order** (`SO:unsorted`) with **every input tag copied byte for byte** and `NM`/`MD`/`AS`/`XS`/`XA` owned by the aligner. Primary = lowest-indexed reference at the best score; the rest of the tie set goes to `XA` (bwa's `ref,±pos,CIGAR,NM;`) and, with `--secondary`, to 0x100 records carrying `SEQ`/`QUAL` as `*` and only the alignment tags + `RG`; MAPQ is 60 unique / 0 tied, nothing in between. Reads below `--min-score` are written unmapped, never dropped. Pipeline: reader thread (raw `bam::Record`s) → bounded channel → dispatcher cutting chunks of ≤ 64 reads / ≤ 16 kb and `spawn`ing each onto rayon (decode, score, trace the chunk's winners together, rewrite, **encode** — through a `bam::io::Writer` over a `Vec<u8>`, since `noodles_bam`'s record encoder is crate-private) → writer thread reordering numbered chunks into BGZF, returning one permit per chunk written (two batches in flight at most). The decode and encode live in the workers because a uBAM record is ~5 kB of `mv`/`MM`/`ML` for a ~130 nt read. **There is no barrier between batches, on evidence:** real tRNA samples carry reads up to ~400 kb, a chunk holding one costs seconds against milliseconds, and a `par_iter` per batch left 16 of 32 threads idle behind those stragglers (30.0 → 18.8 s on a 490 k-read sample, output identical); the per-batch pattern must not come back. `-v` logs the workers' busy time against wall × threads. **`--device` places `Stage::Align`** (#401): on the GPU, a dedicated thread sits between the reader and the dispatcher, takes whole batches off the reader's bounded channel, scores each in one `GpuScorer::score_batch` launch (forward reads, then reverse complements under `--strand both`), and hands the batch on with its `ScoreMatrix` behind an `Arc`; each chunk carries the `Arc` plus its offset, and its worker calls `Aligner::map_reads_scored` instead of `map_reads` — tie selection, pairs-kernel tracebacks, `MD`/`NM` and encoding are the CPU path's own code, so the output is byte-identical (`align_e2e::device_gpu_output_is_byte_identical`; also `samtools view | md5sum`-equal on a 490 k-read sample). A read the kernel does not take comes back unscored and its worker scores it on the CPU. The channel after the GPU thread holds two batches, so memory stays O(`--batch-size`). `device::no_gpu_stage` was removed with its last caller. The GPU thread packs batch *k + 1* while batch *k*'s kernel runs (`GpuScorer::{prepare, submit, finish}`), so the device sits at ~100% whenever it has work; what leaves it idle is the CPU side stalling, and the very long reads (50–400 kb) are what stalls it (dropping the 52 reads over 20 kb takes M1 from 14.3 to 7.5 s): the GPU leaves them to the CPU, where the phase-1 score kernel is, as far as measured, **memory-bound on a read that long** (its per-row `H`/`E` state is 50 MB for a 395 kb read, streamed once per reference column: 4.9 s to score that read on one rna core, 2e9 cells/s), and the chunk holding it blocks the ordered writer while the permit window fills. Splitting such a read's scoring over the lane groups on six threads did not help (its chunk still took 3.4–3.9 s — six streams of the same size at once), and the fix that would — tiling the CPU score kernel's rows so the state stays in cache — is a phase-1 kernel change left for later; a 4× window bought 14.3 → 12.6 s on M1 for +0.7 GB, not taken. `--scoring` defaults to gpu-tRNA-mapper's `2,-1,-10,-1`; bwa `-x ont2d` is `1,-1,-2,-1`, and even then 27 of the 60 classify-fixture reads get a CIGAR different from bwa's (`tests/align_e2e.rs` prints the count; charging calls match the golden bit for bit on every read whose alignment is unchanged).
...
**The cold open, not the read, is what a network filesystem charges for.** A reader is not free once mapped: `signal_extractor()` must recover each signal batch's row count, which is the one field the Arrow IPC footer's block list does not carry, so it reads every batch's own message header — one scattered touch per batch, thousands on a large file, 15–24 ms each cold on BeeGFS. None of it is charged to a read, so it never shows in a reads/s number, and it is paid per file per process; a run over a directory of POD5s can sit for minutes at ~0% CPU before it looks like it has started. Two things address it and they compose: a `.p5s` sidecar carries the geometry so the walk is skipped outright (`escpod index`, and `parse_with_row_counts` *checks* a supplied geometry against the first and last batch rather than believing it), and when there is no sidecar the walk itself runs concurrently — every offset is known before the first header is read, so it was only ever sequential because it was written as a loop. Measured cold on 12 never-touched files of #334's 574 GB set, per read since batch count scales with file size: 112 µs/read serial against 5.5 µs/read parallel, no overlap between the arms, 82.3 s → ~2.6 s on the largest (576 k reads). Major faults track read count in both arms — the same bytes, not one round trip at a time. `ESCAPEPOD_POD5_FOOTER_SERIAL=1` forces the old walk; `examples/open_cost` is the measurement, and times `Reader::open`, `read_index()` and `signal_extractor()` apart because their fixes differ (the reads-table scan behind `read_index()` is front-to-back and the kernel's readahead already serves it, ~120–550 ms per file). The generalising lesson: the probe that measured #334's lookup and read order spent **85–95% of its wall clock outside every timer it had**, in the calls that construct the reader — so time the setup, or the thing you are measuring is not the thing that is slow.
