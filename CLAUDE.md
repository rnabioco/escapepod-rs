...
- `crf/avx2.rs` (x86_64, runtime-dispatched): AVX2+FMA kernels with Cephes-style vector `exp`/`ln`. The decode is transcendental-bound (~770k `exp` + ~260k `ln` per 200-timestep read) and was measured at **half** the total CPU cost — encoder 13.9 ms/read vs scalar decode 13.53 ms, AVX2 decode 2.40 ms (5.6×, 1.7× end-to-end; `cargo bench --bench crf_decode`). This is a **prerequisite** for `crf-gpu`, not a nicety: with the encoder on the device a scalar decode would be essentially the whole runtime. Forward and backward are duals, so each gets one cheap reshape (`expand` replicates each source 4×; `deinterleave4` transposes the 4:1 fold) after which every inner loop is five unit-stride rows. Not bit-identical to scalar (polynomial `exp`/`ln`, reassociated softmax denominator) — the contract is same-sequence, enforced by an equivalence test; the argmax's tie-break runs through the same scalar scan on both paths.
...
- `demux`: Barcode demultiplexing workflow with subcommands:
  ...
  - `basecall`: CTC-CRF barcode basecalling from a boundaries CSV (requires `crf-decode`, in the default build). Emits decoded sequences, **not** barcode calls — assigning a sequence to a reference needs an edit-distance aligner, which this workspace does not have yet.
