...
- **escapepod-align**: all-vs-all read-to-reference alignment for small panels (tRNA) — the engine behind `escpod align` (#395). A leaf crate on purpose: **std only, no noodles, no git dependency**, so unlike `escapepod-demux` it stays `cargo publish`-able; all BAM/FASTA/FASTQ I/O is the CLI's, as with `escapepod-classify`. Written from the issue, with gpu-tRNA-mapper as the *functional* model; its design was reviewed for #415 and nothing was ported (a port would need its Apache-2.0 attribution in a NOTICE). See `crates/escapepod-align/CLAUDE.md` for the layer map (`alphabet`/`Scoring`/`Mode`/`scalar`/`simd`/`sam`/`Aligner`/`cuda`), the two SIMD kernel families, the CUDA score kernel (`gpu`, #401), and the `MD`/`NM` `samtools calmd` contract.
