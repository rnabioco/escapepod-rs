...
- `crf/avx2.rs` (x86_64, runtime-dispatched): AVX2+FMA kernels with Cephes-style vector `exp`/`ln`. The decode is transcendental-bound (~770k `exp` + ~260k `ln` per 200-timestep read) and was measured at **half** the total CPU cost — encoder 13.9 ms/read vs scalar decode 13.5 ms, AVX2 decode 2.3 ms (5.9×, 1.7× end-to-end). This is a **prerequisite** for `crf-gpu`, not a nicety: with the encoder on the device a scalar decode would be essentially the whole runtime. Forward and backward are duals, so each gets one cheap reshape (`expand` replicates each source 4×; `deinterleave4` transposes the 4:1 fold) after which every inner loop is five unit-stride rows. Not bit-identical to scalar (polynomial `exp`/`ln`, reassociated softmax denominator) — the contract is same-sequence, enforced by an equivalence test; the argmax's tie-break runs through the same scalar scan on both paths.
